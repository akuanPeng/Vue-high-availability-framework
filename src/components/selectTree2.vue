<template>
    <div :class="classes" onselectstart="return false">
        <button type="button" class="tree-select-single" @click="open = !open" v-if="(multiple && selecteds.length < 1) || (!multiple && selecteds.length <= 1)">
            ...
            <span v-if="selecteds.length === 1" :title="(titles[selecteds[0]] || { title: '加载中...' }).title">{{(titles[selecteds[0]] || { title: '加载中...' }).title}}</span>
        </button>
        ...
        <div class="tree-select-dropdown">
            <Tree :data="data" :load-data="loadData" :render="renderContent" :children-key="childrenKey" :id-key="mKey"></Tree>
        </div>
    </div>
</template>
<script>
    ...
    computed: {
        classes () {
            return [
                ...
                { 'tree-select-permissionsstatus': true }
            ]
        }
    },
    methods: {
        loadData (item, callback) {
            this.$emit('on-search', item, callback)
        },
        // 判断所有平级节点状态
        judgeState (nodes, id) {
        ...
        renderContent (h, { root, node, data, parent, nodes }) {
            ...
            return h('span', {
                ...
                h('span', {
                    ...
                    style: {
                        color: data.isValid === false ? '#c5c8ce' : '#495060'
                    },
                    on: {
                        // 选中
                        click: e => {
