<template>
    <div class="selecteds-x" @click="onHeaderClick">
        <div class="ivu-tag ivu-tag-checked" v-for="(item, index) in selectedMultiple" :key="index">
            <span class="ivu-tag-text">{{ item.label }}</span>
            <Icon type="ios-close-empty" @click.native.stop="removeTag(item)" v-if="!disabled"></Icon>
        </div>
        <span
        ...
        <Icon type="arrow-down-b" :class="[prefixCls + '-arrow']" :style="{marginRight: 0}" v-if="!resetSelect && !remote && !disabled"></Icon>
    </div>
</template>
<script>
    ...
    export default {
        ...
        computed: {
            ...
            queryText: {
                get () {
                    return this.remote ? (this.query || this.initialLabel) : this.query
                },
                set (value) {
                    this.query = value
                }
