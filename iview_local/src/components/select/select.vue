<template>
    ...
    >
        <div
            ...
            :style="{paddingRight: multiple && isOpt ? '24px' : '24px', maxHeight: '280px', overflow: multiple && isOpt ? 'auto' : 'inherit'}"
        ...
        >
            <slot name="input">
                ...
                <select-head
                    ...
                    @on-input-focus="inputFocusSet(true)"
                    @on-input-blur="isFocused = false"
                    @on-clear="clearSingleSelect"
                />
            </slot>
        </div>
        <div class="ak-selectOpt" ref="selectOpt" :style="{position: 'absolute', left: 0, right: 0, top: '36px', padding: '3px', height: '31px', zIndex: 901, borderBottom: '1px solid #f0f0f0', boxSizing: 'border-box', textAlign: 'center'}" v-show="isOpt && multiple && dropVisible">
            <Button size="small" style="float: left;" @click.prevent.stop="setValues([])">清空</Button>
            <span style="position: relative;top: 6px;" v-if="maxNumber">{{values.length}}/{{maxNumber}}</span>
            <Button size="small" style="float: right;" @click.prevent.stop="setValues(maxNumber ? slotOptions.slice(0, maxNumber).map(_ => _.componentOptions.propsData.value) : slotOptions.map(_ => _.key))">全选</Button>
        </div>
...
<script>
    ...
    const getOptionLabel = option => {
        ...
        const textHTML = option.componentOptions.children.reduce((str, child) => str + (child.children && child.children.length ? (child.children[0].text || '') :''), '');
        return option.componentOptions.propsData.label || textContent || (typeof innerHTML === 'string' ? innerHTML : textHTML);
    };
    ...
    export default {
        ...
        computed: {
            isMaxNumber () {
                return this.values.length >= this.maxNumber
            },
            classes () {
                return [
                    `${prefixCls}`,
            ...
            dropdownCls () {
                return {
                    ...
                    ['ak-dropdown-all']: this.multiple && this.isOpt
                };
            },
            selectionCls () {
                return {
        ...
        methods: {
            inputFocusSet (value) {
                ...
                if (this.multiple && this.isOpt) {
                    setTimeout(() => {
                        if (document.querySelector('.ak-dropdown-all')) {
                            this.$refs.selectOpt.style.top = (5 + parseInt(document.querySelector('.ak-dropdown-all').style.top)) + 'px'
                        }
                    }, 200)
                }
            },
            ...
            processOption(option, values, isFocused){
                ...
                const propsData = {
                    ...
                    selected: isSelected,
                    isFocused: this.isOpt && this.multiple ? false : isFocused,
                    disabled: this.isOpt && this.multiple && this.maxNumber ? (isSelected ? (typeof disabled === 'undefined' ? false : disabled !== false) : this.isMaxNumber) : (typeof disabled === 'undefined' ? false : disabled !== false),
                };
        ...
        watch: {
            ...
            query (query) {
                ...
                const hasValidQuery = (query !== '' || this.maxNumber) && (query !== lastRemoteQuery || !lastRemoteQuery);
                ...
                if ((query !== '' || this.maxNumber) && this.remote) {
